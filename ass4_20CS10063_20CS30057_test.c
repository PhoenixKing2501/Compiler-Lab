enum NODE *createNode(int k, enum NODE *ptr)
{
	enum NODE *temp = malloc(sizeof *temp);
	temp->value = k;
	temp->next = ptr;
 ...
 * @return enum LLIST : empty linked-list
 */
enum LLIST createList()
{
	enum LLIST list = malloc(sizeof *list);
	list->head.value = 0;
	list->head.next = NULL;
	list->tail = &list->head;
 ...
 * @param H : enum LLIST => list on which the operation is to be performed
 * @return int : 1 if empty, 0 otherwise
 */
int isEmptyList(enum LLIST H)
{
